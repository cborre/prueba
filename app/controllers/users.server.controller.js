...
exports.renderSignin = function(req, res, next) {
	// Si el usuario no está conectado renderizar la página signin, en otro caso redireccionar al usuario de vuelta a la página principal
	if(!req.user) {
	...
	if(!req.user) {
		...
		user.save(function(err) {
			...
			req.login(user, function(err) {
				...
				// Redirecionar al usuario de vuelta a la pagina de la aplicación principal
				return res.redirect('/');
			});
		});
	} else {
	...
	User.findOne({
		...
		if(err) {
			...
			if(!user) {
				...
				// Continuá al siguiente middleware
				return done(err, user);
			}
		}
	});
...
// Crear un nuevo método controller para signin out
...
/*exports.create = function(req, res, next) {
	...
	user.save( function(err) {
		if (err) {
			...
			// Usar el objeto 'resaponse' para enviar una respuesta JSON
			res.json(user);
		}
	});
};*/
...
// Crear un método controller 'userByID'
...
/*exports.delete = function(req, res, next) {
	// USamos el método 'remove' de la instancia 'User' para eliminar un documento
